...
public class BasicUtill {
    ...
    public void report() {
        ...
        try{
            connection = DriverManager.getConnection("jdbc:postgresql://localhost:5432/Kebun", "postgres", "rini230320");
            Statement st = connection.createStatement();
            ResultSet resultSet = st.executeQuery("SELECT * FROM hasil_panen");
            while (resultSet.next()){
                String uuid = resultSet.getString("id");
                String komoditas = resultSet.getString("komoditas");
                String total = resultSet.getString("total");
                String created = resultSet.getString("createdat");
                String updated = resultSet.getString("updatedat");
                ...
                rowNUm = rowNUm+1;}
            try {
                FileOutputStream out =  new FileOutputStream(new File("reportHasilPanen\\Laporan"+tanggal+".xlsx"));
                workbook.write(out);
                out.close();
                Log.info("Report has been generated successfully");
                mailer.send(Mail.withText("rinifajarwati14@gmail.com",
                        ...
                        .addAttachment("Laporan hasil kebun"+tanggal+".xlsx",
                                new File("reportHasilPanen\\Laporan"+tanggal+".xlsx"), "text/plain"));
                Log.info("success");
